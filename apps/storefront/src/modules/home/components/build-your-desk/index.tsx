...
function DeskScene({
  ...
  return (
    <svg viewBox="0 0 600 380" xmlns="http://www.w3.org/2000/svg" className="w-full"
      style={{ background: "#F8F5F0", border: "1px solid #D4D4CC" }}>
      <line x1="20" y1="340" x2="580" y2="340" stroke="#EDEDE8" strokeWidth="1" />
      {/* Cabinet */}
      <g opacity={hasCabinet ? 1 : 0} style={{ transition: "opacity .4s" }}>
        <rect x={deskX - 65} y="250" width="55" height="86" fill="#EDEDE8" stroke="#D4D4CC" strokeWidth="1" rx="1" />
        <line x1={deskX - 60} y1="278" x2={deskX - 15} y2="278" stroke="#D4D4CC" />
        <line x1={deskX - 60} y1="306" x2={deskX - 15} y2="306" stroke="#D4D4CC" />
        <circle cx={deskX - 37} cy="264" r="2" fill="#B0B0A8" />
        <circle cx={deskX - 37} cy="292" r="2" fill="#B0B0A8" />
        <circle cx={deskX - 37} cy="320" r="2" fill="#B0B0A8" />
      </g>
      {/* Desk legs */}
      ...
      <rect x={legL - 6} y="333" width="17" height="4" fill="#D4D4CC" rx="1" />
      <rect x={legR - 6} y="333" width="17" height="4" fill="#D4D4CC" rx="1" />
      {/* Desk pad */}
      {hasDeskPad && (
        <rect x={deskX + 20} y={deskY - 2} width={deskW - 40} height={deskDepth - 2}
          fill="#EDEDE8" rx="1" style={{ transition: "opacity .4s" }} />
      )}
      {/* Desk surface */}
      <rect x={deskX} y={deskY} width={deskW} height={deskDepth} fill={topHex} rx="1"
        style={{ transition: "fill .4s, width .5s ease, x .5s ease" }} />
      <rect x={deskX} y={deskY} width={deskW} height={2} fill="rgba(0,0,0,0.06)"
        style={{ transition: "width .5s ease, x .5s ease" }} />
      {/* Laptop stand */}
      <g opacity={hasLaptop ? 1 : 0} style={{ transition: "opacity .4s" }}>
        <rect x={deskX + deskW - 80} y={deskY - 10} width="50" height="3" fill="#B0B0A8" rx="1" />
        <rect x={deskX + deskW - 60} y={deskY - 28} width="10" height="18" fill="#D4D4CC" />
        <rect x={deskX + deskW - 85} y={deskY - 42} width="60" height="14" fill="#EDEDE8" stroke="#D4D4CC" strokeWidth="1" rx="1" />
      </g>
      {/* Monitor arm (single or heavy) */}
      <g opacity={hasMonArm && !hasDualMon ? 1 : 0} style={{ transition: "opacity .4s" }}>
        <rect x={deskX + deskW * 0.3} y={deskY} width="8" height="12" fill="#8A8A82" />
        <rect x={deskX + deskW * 0.3 + 2} y={deskY - 58} width="4" height="58" fill="#B0B0A8" />
        <rect x={deskX + deskW * 0.3 + 4} y={deskY - 58} width="55" height="3" fill="#B0B0A8" />
        <rect x={deskX + deskW * 0.3 + 14} y={deskY - 100} width="90" height="55" fill="#EDEDE8" stroke="#D4D4CC" strokeWidth="1" rx="2" />
        <rect x={deskX + deskW * 0.3 + 19} y={deskY - 96} width="80" height="47" fill="rgba(91,192,235,.08)" rx="1" />
      </g>
      {/* Monitor arm dual */}
      <g opacity={hasDualMon ? 1 : 0} style={{ transition: "opacity .4s" }}>
        <rect x={deskX + deskW * 0.45} y={deskY} width="8" height="12" fill="#8A8A82" />
        <rect x={deskX + deskW * 0.45 + 2} y={deskY - 66} width="4" height="66" fill="#B0B0A8" />
        <rect x={deskX + deskW * 0.45 - 56} y={deskY - 66} width="60" height="3" fill="#B0B0A8" />
        <rect x={deskX + deskW * 0.45 + 6} y={deskY - 66} width="60" height="3" fill="#B0B0A8" />
        <rect x={deskX + deskW * 0.45 - 96} y={deskY - 110} width="90" height="55" fill="#EDEDE8" stroke="#D4D4CC" strokeWidth="1" rx="2" />
        <rect x={deskX + deskW * 0.45 - 91} y={deskY - 106} width="80" height="47" fill="rgba(91,192,235,.08)" rx="1" />
        <rect x={deskX + deskW * 0.45 + 16} y={deskY - 110} width="90" height="55" fill="#EDEDE8" stroke="#D4D4CC" strokeWidth="1" rx="2" />
        <rect x={deskX + deskW * 0.45 + 21} y={deskY - 106} width="80" height="47" fill="rgba(91,192,235,.08)" rx="1" />
      </g>
      {/* Keyboard + mouse */}
      <rect x={deskX + deskW * 0.25} y={deskY - 8} width={deskW * 0.22} height="6" fill="#EDEDE8" rx="1" />
      <ellipse cx={deskX + deskW * 0.55} cy={deskY - 5} rx="8" ry="5" fill="#EDEDE8" />
      {/* Dimension label */}
      <text x={deskX + deskW / 2} y="370" textAnchor="middle" fill="#8A8A82" fontSize="10"
...
export default function BuildYourDesk({
  ...
  return (
    <section className="bg-white" style={{ padding: "clamp(56px, 7vw, 96px) 0" }}>
      <div className="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-10">
        {/* Heading */}
        <div className="text-center mb-10">
          <h2 className="font-display font-bold text-ergo-950 leading-[1.1]"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.3fr_1fr] gap-8 items-start lg:gap-10">
          ...
          <div className="relative">
            ...
            {/* Price badge */}
            <div className="absolute top-4 right-4 px-4 py-3 text-center bg-ergo-sky/10 backdrop-blur-sm">
              <div className="text-[0.65rem] font-semibold text-ergo-400 uppercase tracking-widest">Total</div>
              <div className="text-xl font-bold text-ergo-950 font-display">${total}</div>
            </div>
            {/* Config summary */}
            <div className="mt-3 px-4 py-3 bg-ergo-bg border border-ergo-200/60">
              ...
              {cartSkus.map((item, i) => (
                <div key={i} className="flex justify-between items-center py-1.5"
                  style={{ borderBottom: i < cartSkus.length - 1 ? "1px solid #EDEDE8" : "none" }}>
                  <span className="text-[0.78rem] text-ergo-600">{es ? item.name : item.nameEn}</span>
                  <span className="text-[0.78rem] text-ergo-400 font-medium">${item.price}</span>
                </div>
              ))}
              <div className="flex justify-between items-center pt-2 mt-1 border-t border-ergo-200">
                <span className="text-[0.82rem] text-ergo-950 font-semibold">Total</span>
                <span className="text-[0.82rem] text-ergo-950 font-bold">${total}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-5">
                ...
                {FRAMES.map((f, i) => (
                  <button key={f.id} onClick={() => setFrameIdx(i)}
                    className={`px-3 py-2 text-[0.78rem] font-medium border transition-all duration-150 ${
                      frameIdx === i ? "bg-ergo-sky-dark border-ergo-sky text-white" : "bg-white border-ergo-200 text-ergo-600 hover:border-ergo-400 hover:text-ergo-950"
                    }`}>
                ...
                {FRAME_COLORS.map((c) => (
                  <button key={c.id} onClick={() => setFrameColor(c.id)}
                    className={`flex items-center gap-1.5 px-3 py-1.5 text-[0.75rem] font-medium border transition-all duration-150 ${
                      frameColor === c.id ? "border-ergo-sky text-ergo-950" : "border-ergo-200 text-ergo-600 hover:border-ergo-400"
                    }`}>
                    <span className="w-3 h-3 rounded-full flex-shrink-0 border border-ergo-200" style={{ background: c.hex }} />
                ...
                {(["melamina", "madera"] as const).map((t) => (
                  <button key={t} onClick={() => setTopType(t)}
                    className={`px-4 py-2 text-[0.78rem] font-medium border transition-all duration-150 ${
                      topType === t ? "bg-ergo-sky-dark border-ergo-sky text-white" : "bg-white border-ergo-200 text-ergo-600 hover:border-ergo-400"
                    }`}>
              ...
              <div className="flex flex-wrap gap-2">
                {TOP_SIZES.map((s, i) => (
                  <button key={s.sizeKey} onClick={() => setTopSizeIdx(i)}
                    className={`px-4 py-2 text-[0.78rem] font-medium border transition-all duration-150 ${
                      topSizeIdx === i ? "bg-ergo-sky-dark border-ergo-sky text-white" : "bg-white border-ergo-200 text-ergo-600 hover:border-ergo-400 hover:text-ergo-950"
                    }`}>
                    {s.label}
                ...
                {topType === "melamina" ? (
                  MELA_COLORS.map((col) => (
                    <button key={col.id} onClick={() => setMelaColor(col.id)}
                      className={`flex items-center gap-1.5 px-3 py-2 text-[0.76rem] font-medium border transition-all duration-150 ${
                        melaColor === col.id ? "border-ergo-sky text-ergo-950" : "border-ergo-200 text-ergo-600 hover:border-ergo-400 hover:text-ergo-950"
                      }`}>
                      <span className="w-3.5 h-3.5 rounded-full flex-shrink-0 border border-ergo-200" style={{ background: col.hex }} />
                  ...
                  WOOD_TYPES.map((w) => (
                    <button key={w.id} onClick={() => setWoodType(w.id)}
                      className={`flex items-center gap-1.5 px-3 py-2 text-[0.76rem] font-medium border transition-all duration-150 ${
                        woodType === w.id ? "border-ergo-sky text-ergo-950" : "border-ergo-200 text-ergo-600 hover:border-ergo-400 hover:text-ergo-950"
                      }`}>
                      <span className="w-3.5 h-3.5 rounded-full flex-shrink-0 border border-ergo-200" style={{ background: w.hex }} />
            ...
            <div>
              ...
              <div className="flex flex-col gap-1">
                {ACCESSORIES.map((a) => {
                  ...
                  return (
                    <div key={a.id} className="flex items-center justify-between py-2 px-1 border-b border-ergo-100">
                      <span className="text-[0.8rem] text-ergo-600">
                        {es ? a.label : a.labelEn}
                        <span className="text-ergo-400 ml-1.5">+${a.price}</span>
                      </span>
                      <button onClick={() => toggleAcc(a.id)}
                        className={`relative w-10 h-5 rounded-full transition-colors duration-200 focus:outline-none ${isActive ? "bg-ergo-sky-dark" : "bg-ergo-200"}`}
            ...
            <div className="flex items-center justify-between pt-3 gap-4">
              <div>
                <div className="text-[0.68rem] text-ergo-400 uppercase tracking-widest">{es ? "Total del setup" : "Setup total"}</div>
                <div className="text-2xl font-bold text-ergo-950 font-display">${total}</div>
              </div>
